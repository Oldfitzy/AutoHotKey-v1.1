...
SetTimer, CheckTime, 50000	;How often, in milliseconds, to check if the file has updated - 300000 for 5 mins

ProdMainDir := "\\cmutual.com\fsroot\MPR\IT\ITSD Scripts\AutoHotKey"
StatsFolder := "\\cmutual.com\fsroot\MPR\IT\IT Service Desk\Teamwork\ITSDUtil Stats"
MessagesDir := "\\cmutual.com\fsroot\MPR\IT\ITSD Scripts\AutoHotkey\Data\Messages\"
...
If !FileExist("C:\AHKLocal\") ;if the file directory does not exist then it is created and the PS1 files are pulled in
{
	...
	Try 
	{
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctValidation.ps1, C:\AHKLocal 
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSPwordReset.ps1, C:\AHKLocal
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctUnlock.ps1, C:\AHKLocal
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctFindMD.ps1, C:\AHKLocal
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSReturnCreds.ps1, C:\AHKLocal
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSRemoteAccess.ps1, C:\AHKLocal
	}
	Catch
...
{
	;This location will need changed back to L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\FILENAMEHERE.PS1
	FileGetTime, AcctValLAN, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctValidation.ps1, M
	FileGetTime, PwordResLAN, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSPwordReset.ps1, M
	FileGetTime, AcctUnlLAN, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctUnlock.ps1, M
	FileGetTime, CmdCmmLAN, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctFindMD.ps1, M
	FileGetTime, GetCredsLAN, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSReturnCreds.ps1, M
	FileGetTime, GetRemALAN, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSRemoteAccess.ps1, M
	...
	If (AcctValLAN > AcctValLocal)
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctValidation.ps1, C:\AHKLocal, 1
	If (PwordResLAN > PwordResLocal)
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSPwordReset.ps1, C:\AHKLocal, 1
	If (AcctUnlLAN > AcctUnlLocal)
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctUnlock.ps1, C:\AHKLocal, 1
	If (CmdCmmLAN > CmdCmmLocal)
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSAcctFindMD.ps1, C:\AHKLocal, 1
	If (GetCredsLAN > GetCredsLocal)
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSReturnCreds.ps1, C:\AHKLocal, 1
	If (GetRemALAN > GetRemALocal)
		FileCopy, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\PSRemoteAccess.ps1, C:\AHKLocal, 1
}
...
;Try and catch for running PS??
;Check if last thing in PS was run correctly
;$? | Out-File -filepath C:\AHKLocal\LastThing.txt
;DetectHiddenWindows, On

;Set the initial state for the Magic Button so that it only clears the Context Window.
MagicButton = ClearMagCont

ITSDUtilAdmin := FolderWriteAccess("L:\MPR\IT\ITSD Scripts") ;Checks to see if the user has write access to the AutoHotkey folder. ITSDUtilAdmin = 1/true = yes, 0/false = no.
...
BarNames := ["External", "Password Issues", "Windows", "Outlook", "VPN and VIP", "Okta", "VMware", "Security", "MFP", "ADUC and LAN", "Current"]
...
If (MarqueeFullSize = true)
	Gui, MagWinMarq: Show, x%CurrentMarqCoordinatesX% y%CurrentMarqCoordinatesY% h270 w226, ITSDMarquee
Else 
	Gui, MagWinMarq: Show, x%CurrentMarqCoordinatesX% y%CurrentMarqCoordinatesY% h18 w226, ITSDMarquee
...
If (MagicContextShow = true)
	Gui, MagWinCont: Show, x%CurrentContCoordinatesX% y%CurrentContCoordinatesY% w253, ContextTips
Else
{
	Gui, MagWinCont: Hide
}
...
;Gui, AIOCenter: % ((AIOAlwayOnTop = true) ? +AlwaysOnTop : ) -Border 
;Gui, % ((AIOAlwayOnTop = true) ? (AIOCenter`:+AlwaysOnTop -Border) : (AIOCenter`:-Border))
...
Gui, AIOCenter: Add, Button, Center x5 y24 w30 H21 gCmutualM, M ;coming later cmutualM domain search
Gui, AIOCenter: Add, Button, Center x5 y45 w30 H21 gCmutualD, D ;coming later cmutualD domain search
Gui, AIOCenter: Add, Text, Center x40 y60 w120 vOtherDomain, Domain: Cmutual ;displays the search domain, defual is cmutual
...
Gui, AIOCenter: Add, Picture, x130 y119 w65 h-1 vAccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIOStatus.png ;account status display. unlcoked, locked, etc
...
;Gui, UtilOptions: Add, Checkbox, % ((UtilAlwaysOnTop=true) ? "Checked" : "") x248 y55 vUtilAlwaysOnTop, Always On Top?
If (UtilMarqAlwaysOnTop = true)
...
;Gui, UtilOptions: Add, Checkbox, % (UtilMarqAlwaysOnTop=true) ? "Checked" : "" x248 y77 vUtilMarqAlwaysOnTop, Always On Top?
If (MagicContextShow = true)
...
;Gui, UtilOptions: Add, Checkbox, % ((MagicContextShow=true) ? "Checked" : "") x248 y101 vMagicContextShow, Autohide?
If (AIOAlwayOnTop = true)
...
;Gui, UtilOptions: Add, Checkbox, % ((AIOAlwayOnTop=true) ? Checked : Unchecked) x248 y125 vAIOAlwayOnTop, Always On Top?
...
;Run, PowerShell.exe -NoProfile -NonInteractive -NoLogo -NoExit, C:\AHKLocal\, Hide, PSOutVar
;Run, PowerShell.exe -NoProfile -NoLogo -NoExit -Command $cr = $host.ui.PromptForCredential(""""AHK Credentials""""`, """"Enter your LSA user name and password.""""`, """"$env:userdomain\$env:username-LSA"""" `, """"""), C:\AHKLocal\,, PSOutVar
Run, PowerShell.exe -NoProfile -NoLogo -NoExit -Command $cr = .\PSReturnCreds.ps1, C:\AHKLocal\, Hide, PSOutVar
;Run, PowerShell.exe -NoProfile -NoLogo -NoExit -Command $cr = get-credential, C:\AHKLocal\,, PSOutVar
SetKeyDelay, 5, 0

Return

;Creds = $host.ui.PromptForCredential("AHK Credentials", "Enter your LSA user name and password.", "$env:userdomain\$env:username-LSA", "")
LaunchEmailThing:
...
;faster way to send commands to PS??? probably wrong but a start 
;Control, EditPaste, cd C:\AHKLocal\, WindowsPowershell, ahk_exe Powershell.exe
...
FolderWriteAccess( Folder ) {
    ...
    TempFile := A_WorkingDir . "\temp_write_access_" . %A_Tick% . ".tmp" 

    ; Attempt to write to the temporary file
    FileAppend,, %TempFile% 

    ; Check for write access based on the result
    rval := !ErrorLevel 
...
JumpDescFill:
	FocusJump(7, 26, "*Short Description:", "Cherwell Service Management")
...
AutoPaster:
	...
	SetTitleMatchMode, 2
	WinGet, CWNum, count, Cherwell Service Management (licensed to CMFG Life Insurance Company)
	If CWNum > 1
	{
		WinWaitActive, Cherwell Service Management (licensed to CMFG Life Insurance Company), , 5
		If ErrorLevel
			Return
		Else
		{
	...
	{
		CoordMode, Pixel, Window
		WinActivate, Cherwell Service Management (licensed to CMFG Life Insurance Company)
		ImageSearch, , , 0, 0, %A_ScreenWidth%, %A_ScreenHeight%, %A_ScriptDir%\Data\ImgSrchSrce\CheckedBox.png
		If(ErrorLevel == 0)
			Gosub, Unchecker
		SendInput ^+k
...
;The same as StatGrabber2 but corrects the StatType for certain situations that can't set the StatType variable for some reason.
StatGrabber2:
	WeekOf := WeekPrevSun()
...
ITSDToolbarButton?:
	Run, "L:\MPR\IT\ITSD Scripts\AutoHotKey\Hotkeys Guide.pdf"
Return
...
;Below is for Dev locations, when launching from Dev
...
#Include %A_ScriptDir%\Data\DropDownList\Okta\Master\Master.txt
...
#Include %A_ScriptDir%\Data\DropDownList\Salesforce\Master\Master.txt
...
OnSelectMCM10:
	...
	If %MCMSelected% not in Passwords,Support,Quick Reference,Access Requests,Installations ;Checks to make sure the selection is not a header, if so it does nothing 
	{
		...
		Gosub, %CleanList%
;		Gosub, NoteBox
;		Gosub, AutoPaster	;set up so that secondary paste will fill out multiple fields in cherwell from the Short Description field.
	}
Return
...
OnSelect10:
	...
	If %GetSelected% not in External,Password Issues,Windows,Outlook,VPN and VIP,Okta,VMware,OneDrive,Web Conference,Security,MFP,ADUC and LAN,Hardware,Current ;Checks to make sure the selection is not a header, if so it does nothing 
	{
...
;	Gui, MClickMenu: Hide
	SetControlDelay -1
	ControlClick, Search Open Incidents, Cherwell Service Management (licensed to CMFG Life Insurance Company)
	ControlClick, Search Known Errors, Cherwell Service Management (licensed to CMFG Life Insurance Company)
	ControlClick, Search All KCS Articles, Cherwell Service Management (licensed to CMFG Life Insurance Company)
	ControlClick, Search ROC KCS Articles, Cherwell Service Management (licensed to CMFG Life Insurance Company)
...
;	{
;		FileRead, MWMessage%A_Index%, %A_LoopFileFullPath%
;		MWMessagePath%A_Index% = %A_LoopFileFullPath%
;		MWMTitle%A_Index% = %A_LoopFileName%
...
UpdateMessages:
	...
	Loop, Files, %MessagesDir%\*.txt
	{
;		MWMDate%A_Index% := A_LoopFileTimeModified
...
MagCharCheck:
	Gui +LastFound
	Gui, Submit, NoHide
	If AddTitleEdit Contains /,\,:,*,?,",<,>,|,& ;"   ;Characters that are not allowed
	{
		SendInput, {BackSpace}
		Goto, MagCharCheck
	}
...
AddSaveB:
	...
	Gui, AddMarqWin: Destroy
;	If (PlayMarquee = true) ;If already set to play mode, unpause the message scroller
...
;	ContextTipModDate := SubStr(ContextTipModDate,1,8)
	TwoWeeksAgo += -14, Days
	NewFlag := (ContextTipModDate > TwoWeeksAgo) ? "*New* " : ""
;	If (ContextTipModDate > TwoWeeksAgo)
;		NewFlag := "*New* "
;	Else
;		NewFlag := ""
...
~MButton::
	...
	MCMNeedle := "Cherwell Service Management (licensed to CMFG Life Insurance Company)"
...
^+s:: ;when control, shift and 'i' are pressed together, do the following:
ifWinActive, Cherwell Service Management ;only if Cherwell is active. Key combination will be disabled in other apps.
{
    Send ^{END}
    Send {Enter}
    Send Credential ID: 
...
^+x:: ;when control, shift and x are pressed together, do the following:
	FocusJump(7, 24, "*Customer:", "Cherwell Service Management") ;uses focusjump to find the users id
	SendInput ^a
	SendInput ^c
	Sleep 100
...
^+j:: ;hot key to grab the username from cherwell and put it into the gui
	...
	;NameDisplayString := (Clipboard = "") ? "Enter Name" : ""
...
^+o:: ;when control, shift and 'o' (the letter, not zero) are pressed together, do the following:
ifWinActive, Cherwell Service Management ;only if Cherwell is active. Key combination will be disabled in other apps.
{
    Send ^{END}
    Send {Enter}
    Send Original Caller: 
    Send {Space}
}
Return

^+m:: ;when control, shift and 'm' are pressed together, do the following:
ifWinActive, Cherwell Service Management ;only if Cherwell is active. Key combination will be disabled in other apps.
{
    Send ^{END}
    Send {Enter}
...
^+p::
	...
	SetControlDelay -1
	ControlClick, Search All KCS Articles, Cherwell Service Management (licensed to CMFG Life Insurance Company)
	ControlClick, Search NOC KCS Articles, Cherwell Service Management (licensed to CMFG Life Insurance Company)
	WinActivate, UtilMClickMenu ;The controlclicks happen so fast, Windows doesn't recognize that the middle click menu was ever active and AHK closes it as if it never became active.
	Sleep, 100
	ControlFocus, %DescFieldName%, Cherwell Service Management
Return
...
^Enter::
	ControlClick, Next: Close this Record, Cherwell Service Management (licensed to CMFG Life Insurance Company)
Return
...
SearchID:
	SearchingDomain := "cmutual.com"
	;Maybe other stuff??
	Gosub, SuperSearch
Return
		
CmutualD:
	SearchingDomain := "cmutuald.com"
	;Maybe other stuff??
	Gosub, SuperSearch
Return

CmutualM:
	SearchingDomain := "cmutualm.com"		
	;Maybe other stuff??
	Gosub, SuperSearch
Return
...
SuperSearch:
	...
	AccountToGetInfo := Format("{:Ls}", AccountToGetInfo)
	GuiControl, AIOCenter:Text, OtherDomain, Domain: %SearchingDomain% ;displays domain as cmutual, fututre will also say cmutualD and cmutualM
	...
	GuiControl, AIOCenter:, AccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIOWorking.png ;sets status pic as working, stays until PS script is ran, then it changes
	...
	If (SearchingDomain = "cmutual.com")
	{
		Loop, Read, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\Z All Combined Users and Computers.txt ;loops text file of all machines looking for username that was inputted
		{
	...
	If (SearchingDomain != "cmutual.com")
		Gosub, SpecialDomainLookUp ;if the domain is not cmutual goes to sub routine to find if multiple accounts 
	Else 
		Gosub, LookingForSomethingMore ;if cmutual continues like normal, searching only on the one account 
Return	
...
SpecialDomainLookUp:
	...
	FindMDScript := ".\psacctfindmd.ps1 -acctname " AccountToGetInfo " -sdom " SearchingDomain " -cred $cr" ;PS command to see if multiple users
	...
	While ! Instr(TotalFoundUsers,"Done") ;while not done, keep reading file until Done is read
	{
		...
		{
			GuiControl, AIOCenter:, AccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIONotFound2.png
		...
		{
			FileReadLine, AccountToGetInfo, C:\AHKLocal\CMDDSQR.txt, 1 ;reads name printed on line 1. Reads name incase you search on UserID and the only found account is UserID-Test01-cmm
...
LookingForSomethingMore: 
	DetectHiddenWindows, On
	ValidAccountScript := ".\psacctvalidation.ps1 -acctname " AccountToGetInfo " -sdom " SearchingDomain " -cred $cr"
...
SomethingMoreAfterCtrlSend: 
	...
	While ! Instr(ValidLockStatus,"Done") ;While done is not present, keeps reading file
	{
		...
		{
			GuiControl, AIOCenter:, AccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIODisabled.png
		...
		{ 
			GuiControl, AIOCenter:, AccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIOUnlocked.png
			Break
		}
		...
		{ 
			GuiControl, AIOCenter:, AccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIOLocked.png
			GuiControl, AIOCenter:, UpdateInfo, Working on Unlocking The Account
			;PercentDoneWithChange ("UnlockStatus", "Unlock")
			Gosub, UnlockAfterCtrlSend ;make after ctrl send??
			Break
		}
		Sleep 50
	}
...
CancelCMMD:
CMMDChooseGuiEscape: ;if the mulitple accoutns gui window is closed, display changed to reflect that
	Gui, CMMDChoose: Destroy
	GuiControl, AIOCenter: Text, UpdateInfo, Account Lookup Canceled	
	GuiControl, AIOCenter:, AgeOfPassword, Canceled
	GuiControl, AIOCenter:, AccountStatusPic, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\AIOStatus.png
Return 
...
UserComputerSelection:
	...
	Else 
	{
		If A_GuiControlEvent <> DoubleClick ;if event is not a double click then do nothing
			Return
		Gosub, RemoteControlGo ;gosub on remoting intto selected user computer 
	}	
Return 
...
UnlockAccount: 
	...
	Else 
	{
		...
		UnlockAllDCScript := ".\psacctunlock.ps1 -acctname " AccountToGetInfo " -sdom " SearchingDomain " -cred $cr"
...
UnlockAfterCtrlSend:
	...
	While ! Instr(AccountUnlockStatus,"Done")
	{
  	...
  	DCUnlockcomplete := 0
  	DCUnlockerror := 0
  	FileReadLine, UnltotalDC, C:\AHKLocal\UnlockStatus.txt, 1 ;make this only happen once??
...
ResetPassword: ;Make a spot to show the password, can also be changed here. Also make checkbox to force change at next log in
	...
	Else
	{
		...
		If (!InStr(AccountToGetInfo, "-lsa") && (SearchingDomain = "cmutual.com"))
		{
...
GeneratePassword:
		Random, CapWord, 4, 45 ;selects a random number, between lines 1 and 45
		Random, SymWord, 46, 51 ;selects a random number, between lines 43 and 50
		Random, NumWord, 1000, 2000 ;selects a random number, between lines 100 and 199
		NumWord := Format("{1:02i}",NumWord) ;if number is triple ditits adds a preceiding zero
		FileReadLine, OneWord, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\Password Options.txt, %CapWord% ;gets the number generated before and goes to that line in the file, pulling that word;	FileReadLine, TwoWord, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\Password Options.txt, %LowWord% ;gets the number generated before and goes to that line in the file, pulling that word
		FileReadLine, SymbolWord, L:\MPR\IT\ITSD Scripts\AutoHotKey\Data\Password Options.txt, %SymWord% ;gets the number generated before and goes to that line in the file, pulling that word
...
DoChangePWord:
	...
	DetectHiddenWindows, On
	ChangePasswordScript := ".\pspwordreset.ps1 -acctname " AccountToGetInfo " -newpword " PassChange " -sdom " SearchingDomain " -f " ForceResetCheckBox " -cred $cr"
	ControlSendRaw, ,%ChangePasswordScript%, ahk_pid %PSOutVar%
	ControlSend,,{Enter}, ahk_pid %PSOutVar%
	DetectHiddenWindows, Off 
	
;	PercentDoneWithChange ("PWordSetStatus", "Password Reset")
;	
...
PasteComp: ;pastes the computer name into the cherwell where the last username was pulled from 
	...
	Else 
	{
		...
		Sleep 100
		;SendInput Computer: %ComputerSelection%
...
CompFileExplorer: ;open the C: drive of th eslected computer in windows explorer 
...
DoCompareUsers:
	...
	If (RecursiveCheckBox = false)
		Run, powershell.exe -Command "& '%A_ScriptDir%\Data\Comparinator9000.ps1' -user1 %CompareFirstUser% -user2 %CompareSecondUser% -recursive false",,Hide
	Else
		Run, powershell.exe -Command "& '%A_ScriptDir%\Data\Comparinator9000.ps1' -user1 %CompareFirstUser% -user2 %CompareSecondUser% -recursive true",,Hide
...
RemoteControlGo: ;when select to remote in. Gets the computer, adds a stat, and then rums it in Configuration Manager Remote Control
	...
	ControlSend,, {Enter}, ahk_pid %PSOutVar%
	DetectHiddenWindows, Off
	;Run, \\vmpsccm12p01\RemoteControlv2\CmRcViewer.exe %ComputerSelection%
...
^L::
^+L::
	Run, powershell.exe -Command "& '%A_ScriptDir%\Data\LANFolderRoleGroups.ps1'"
Return

^w::
^+w::
	Gui, AIOCenter: Submit, NoHide
	Run, powershell.exe "& '%A_ScriptDir%\Data\CUserScripts\LANFileMove.ps1' '%ComputerSelection%' $cr"
...
::byes::
	SendInput Please click "yes" when prompted, or click on the shield at the bottom and then click "yes".
	SendInput {Enter}
Return

::bnoresp::
	SendInput If you are needing any further assistance with this issue, please contact the IT Service Desk again. Thanks and have a great day!
	SendInput {Enter}
Return
...
::bpass::
	SendInput Could you please provide your passphrase for verification?
	SendInput {Enter}
Return
...
;	Run %A_ScriptDir%\Data\Batch\CleanWinPos.bat,,hide
